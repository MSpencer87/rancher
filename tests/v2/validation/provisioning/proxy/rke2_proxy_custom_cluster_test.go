...
func (r *ProxyRKE2CustomClusterTestSuite) TestProvisioningRKE2CustomClusterProxy() {
	...
	tests := []struct {
		...
		{provisioninginput.AdminClientName.String() + "-" + permutations.RKE2AirgapCluster + "-", r.client},
	}
	for _, tt := range tests {
		permutations.RunTestPermutations(&r.Suite, tt.name, tt.client, r.clustersConfig, permutations.RKE2AirgapCluster, nil, r.corralPackage)
	}
}

// In order for 'go test' to run this suite, we need to create
// a normal test function and pass our suite to suite.Run
func TestProxyCustomClusterRKE2ProvisioningTestSuite(t *testing.T) {
